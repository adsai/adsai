...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a ADSAI address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no ADSAI: URI
...
{
    // Convert adsai:// to adsai:
    //
    //    Cannot handle this later, because adsai:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open adsai.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ADSAI.lnk";
}
...
{
    return GetAutostartDir() / "adsai.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a adsai.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=ADSAI\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the adsai app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add adsai app to startup item list
